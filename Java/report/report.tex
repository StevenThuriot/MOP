...
\section{Introduction}
	...
	\section{System Operations}
		\subsection{Task Management}
			...
			If this is the case, the user had 2 options: Remove all the tasks recursively, or only remove the selected task and remove binding (dependencies) to other tasks.
			\begin{figure}[H]
				\begin{center}
			...
			The application also makes sure Tasks are kept at a consistent state. For example, if a task is marked in a state, this is notified to other, dependant tasks so they can make the possibly necessary adjustments so they comply with all the businessrules. This happens recursively.
		...
		\subsection{Project Management}
			...
			A project can be removed without too many details. If the user wants to remove a project, all of the tasks connected to that project are removed recursively. The user simply selects a Projects and all of the underlying tasks/dependent tasks are removed with it.
			\begin{figure}[H]
				\begin{center}
